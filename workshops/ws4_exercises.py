...
def reverse(string_to_reverse):
    reversed_string = ''
    for character in string_to_reverse[::-1]:
...
def is_member(value, list_to_check):
    for list_value in list_to_check:
        if list_value == value:
            return True
    return False
...
def overlapping(original_list, list_to_check_for_overlapping):
    for list_value in list_to_check_for_overlapping:
        if is_member(list_value, original_list):
            return True
    return False
